...
for (ind in 1:length(algs)){
  alg <- algs[ind]
  load(sprintf("/Users/nurdankuru/Desktop/Paper_ROC_Measures_%s.RData", alg))
  ...
  for (close in closes){
    for (thr in thrs){
      ...
      p <- ggplot(df, aes(x = Algorithm, y = Value, color = Algorithm)) +
        ...
        geom_signif(comparisons = list(c("PHACE", "DCA")), textsize = 8, vjust = -0.5, step_increase = 0.05, map_signif_level = TRUE, color = "black") +
...
ggsave(sprintf("All_ROC_Paper_Pairwise.pdf"), combined_final, width = ( 210/25.4), height = 4)
